...
public class Level2 implements LevelOfWordsToLearn{
	DataMediator m;
	private final int level = 2;
	private final int ls_size=15;
	private final int points=50;
	
	public Level2(DataMediator m)
	{
		this.m = m;
	}
	...
	public LearningSet generateWords(String language, int userLevel) 
	{
		...
		if(userLevel==1) ile=(int)(0.1*ls_size);
		else if(userLevel==2) ile=(int)(0.2*ls_size);
		else ile=(int)(0.4*ls_size);
		...
		if(size<ile) ile=size;
		//jesli na levelu 2 jest mniej slow niz potrzeba to zwroci mniej niz ls_size slow		
